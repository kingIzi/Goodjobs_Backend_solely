...
from utilities.send_sms import send_sms_email, send_sms_message
...
def verify_signup_otp(request):
    try:
        ...
        try:
            ...
            print(e)
            return JsonResponse({'status': 'error', 'message': "Failed to retrieve OTP code and phone number", 'status_code': 404, }, status=404)
    except:
        return JsonResponse({'status': 'An error occurred on the server'},status=500)
...
def login(request):
    try:
        ...
        if status == 'staff' and user.is_staff:
            ...
            message = f'Weka token ili kuendelea. Token yako ni {otp_value}'
            print(message)
            #send_sms_message(form.cleaned_data.get('phone_number'), message)
            return JsonResponse({'status': 'success', 'message': 'Please input otp to complete authentication',
                                        'status_code': 200}, status=200)
        else:
